...
function possibleValidEquation(currVal: number, numbers: number[], index: number, targetVal: number): boolean {
  ...
  const concat = parseInt(`${currVal}${nextNumber}`, 10);

  return (
    possibleValidEquation(sum, numbers, index + 1, targetVal) ||
    possibleValidEquation(product, numbers, index + 1, targetVal) ||
    possibleValidEquation(concat, numbers, index + 1, targetVal)
  );
}
